...
process.on("unhandledRejection", (err, p) => {
  console.error(`Unhandled Rejection at: ${JSON.stringify(p, null, 4)}, reason: ${JSON.stringify(err, null, 4)}`);
  console.error(`File gây lỗi: ${p?.fileName || 'Không xác định'}, dòng gây lỗi: ${p?.lineNumber || 'Không xác định'}`);
});
process.on("uncaughtException", (err, p) => {
  console.error(`Uncaught Exception at: ${JSON.stringify(p, null, 4)}, reason: ${JSON.stringify(err, null, 4)}`);
  console.error(`File gây lỗi: ${p?.fileName || 'Không xác định'}, dòng gây lỗi: ${p?.lineNumber || 'Không xác định'}`);
});
